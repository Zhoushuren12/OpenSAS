...
class DataProcessing:
            ...
            for i in range(1, 10000):
                ...
                if Path.exists(self.root/subfolder):
                    ...
                    try:
                        ...
                        logger.warning(f'找不到 Status.dat文件 ({file_name})')
                else:
                    break
    ...
    def _read_PFA(self, print_result: bool):
        ...
        for idx_gm in range(self.GM_N):
            ...
            while True:
                ...
                if not Path.exists(folder):
                    break
                t = np.loadtxt(folder/'Time.out')[10:, 0]
                current_dir = Path(__file__).parent
                gm_dir = current_dir.parent / 'GMs'
                gm_info_path = gm_dir / 'GM_info.json'
                with open(gm_info_path , 'r') as f:
                    dt_dict = json.loads(f.read())
                dt = dt_dict[gm_name]
                Gm_A = np.loadtxt(gm_dir / f'{gm_name}.txt')
                Gm_T = np.arange(len(Gm_A)) * dt
                a_base = self._uniform_time_seires(Gm_T, Gm_A, t)                
    ...
    def read_pushover(self, H: float, FDR_step=[0.01*i for i in range(1, 11)], plot_result=True):
        ...
        if not PO_path.exists():
            PO_path = self.root / 'pushover'
        Time = np.loadtxt(PO_path / 'Time.out')[11:]
        n = len(Time)
        # 侧向力分布模式
        pattern = np.loadtxt(PO_path / 'Pattern.out')
        ...
        for i in range(self.Nstory):
            u[i] = np.loadtxt(PO_path / f'Disp{i+2}.out')[11:]
        # 层间位移角
        IDR = np.zeros((self.Nstory, n))
        for i in range(self.Nstory):
            IDR[i] = np.loadtxt(PO_path / f'SDR{i+1}.out')[11:]
        # 归一化楼层位移角(各层位移除以结构总高)
        ...
        np.savetxt(self.root_out/'归一化侧向力分布.out', pattern)
...
if __name__ == "__main__":
    ...
    output_folder = Path(r'D:\Study\My_Project\OpenSAS-SMAPFDB\Output_data\MC8_SMAPFDF\MC8_TH_DBE_data')
