...
class MRF:
    ...
    def __init__(self, model_name: str, Nstory: int, Nbay: int,heights:list, script: Literal['tcl', 'py']='tcl', notes=''):
        """实例化分析模型
    ...
    @staticmethod
    def least_squares_scale(a: np.ndarray, b: np.ndarray) -> float:
        """Return the scale factor f that minimizes mean((a * f - b) ** 2)."""
        denominator = np.dot(a, a)
        if denominator == 0:
            raise ZeroDivisionError('无法根据零反应谱计算缩放系数')
    ...
    def scale_ground_motions(self,
        ...
        for idx, gm_name in enumerate(self.GM_names):
            ...
            if method == 'a':
                ...
                SF = self.least_squares_scale(RSA[idx1: idx2], Sa_code[idx1: idx2])
            elif method == 'd':
                PGA = para
                SF = PGA / max(abs(th))
            elif method == 'e':
    ...
    def set_running_parameters(
        ...
        with open(f'{self.Output_dir}/ground_motions.dat', 'w') as f:
            text = '\n'.join(self.GM_names)
            f.write(text)
        with open(f'{self.Output_dir}/heights.dat', 'w') as f:
            text = ' '.join(str(h) for h in self.heights)
            f.write(text)
    ...
    def run_time_history(self, print_result=False, collapse_limit: float=0.1, parallel: int=0, block=False):
        """运行时程分析
    ...
    def run_IDA(
            ...
            max_ana=30, test=False, intensity_measure: Literal[1, 2]=1, T_range: tuple=None,
             ...
             resume: bool=True, checkpoint_name: str='_ida_checkpoint.json'):
        """IDA分析
        ...
        self.IDA_resume = resume
        self.IDA_checkpoint_name = checkpoint_name
        self.collapse_limit = collapse_limit
        if not isinstance(parallel, int) or parallel < 0:
            self.logger.error('参数parallel格式错误，必须为大于等于0的整数')
            raise ValueError('parallel格式错误')
        ...
        if self.display and parallel > 0:
            self.logger.warning('采用多进程并行计算时，暂不支持显示实时动画')
            self.display = False
        # IDA only needs the spectrum values used by the selected intensity measure.
        if intensity_measure == 1:
            T = np.array([T0], dtype=float)
        elif intensity_measure == 2:
            if T_range is None:
                raise ValueError('intensity_measure=2时必须提供T_range')
            Ta, Tb = T_range
            T = np.arange(Ta, Tb + 0.005, 0.01)
        else:
            raise ValueError('intensity_measure必须为1或2')
        self.T = T
    ...
    #         os.makedirs(self.dir_log)
    #     self._init_set_QApp()
    #     self._gui_window = MyWin(self, running_case, IDA_para=IDA_para, print_result=print_result)
