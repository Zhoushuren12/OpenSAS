...
TEMPERATURE_DATA_DIR = Path(
    r"C:\Users\Admin\Desktop\考虑温度效应的PFSD\区域温度分析\02_Yushu_Turpan_Wenchang\Temperature\daily_temperature_1974_2024"
)
RESULT_DIR = PROJECT_ROOT / "Paint" / "Regional_Yushu_Turpan_Wenchang_PFSDF"
...
CITY_CONFIGS = [
    {"city": "Yushu", "label": "Yushu", "color": "#2f8de4"},
    {"city": "Turpan", "label": "Turpan", "color": "#f06a3a"},
    {"city": "Wenchang", "label": "Wenchang", "color": "#38a169"},
]
...
def find_fragility_workbook(model: str, temperature: float, edp: str) -> Path:
    frag_dir = find_case_directory(model, temperature) / "MC8_IDA_data_frag"
    candidates = [
        path
        for path in sorted(frag_dir.glob(f"*_{edp}.xlsx"))
        if "IDA" not in path.name and "概率需求模型" not in path.name
    ]
    if not candidates:
        raise FileNotFoundError(f"Missing fragility workbook for {model} {temperature:g} C {edp}.")
    return candidates[0]


def read_temperature_fragility_curve(
    model: str,
    temperature: float,
    edp: str,
    damage_state: DamageState,
    ...
    workbook = find_fragility_workbook(model, temperature, edp)
    df = pd.read_excel(workbook, skiprows=1, header=0)
    if damage_state.name not in df.columns:
        raise KeyError(f"{workbook.name} does not contain {damage_state.name}.")

    x = df.iloc[:, 0].astype(float).to_numpy()
    y = df[damage_state.name].astype(float).to_numpy()
    ...
    x = x[order]
    y = y[order]
    return np.interp(im_values, x, y, left=y[0], right=y[-1])


def fill_missing_temperature_curves(curves: dict[float, np.ndarray | None]) -> dict[float, np.ndarray]:
    available_temperatures = np.array(
        [temperature for temperature, curve in curves.items() if curve is not None],
        dtype=float,
    )
    if available_temperatures.size == 0:
        raise ValueError("No valid fragility curves are available for interpolation.")

    filled_curves: dict[float, np.ndarray] = {}
    for temperature, curve in curves.items():
        if curve is not None:
            filled_curves[temperature] = curve
            continue

        lower = available_temperatures[available_temperatures < temperature]
        upper = available_temperatures[available_temperatures > temperature]
        if lower.size and upper.size:
            t0 = float(lower.max())
            t1 = float(upper.min())
            weight = (temperature - t0) / (t1 - t0)
            filled_curves[temperature] = curves[t0] * (1.0 - weight) + curves[t1] * weight
        else:
            nearest = float(available_temperatures[np.argmin(np.abs(available_temperatures - temperature))])
            filled_curves[temperature] = curves[nearest]
        print(f"[fill] Missing fragility curve at {temperature:g}{CELSIUS}; interpolated from neighboring temperatures.")
...
def compute_regional_fragility_results(
    ...
    for edp, damage_states in DAMAGE_STATES.items():
        ...
        for damage_state in damage_states:
            raw_curves = {
                float(temp): read_temperature_fragility_curve(
                    model,
                    float(temp),
                    edp,
                    damage_state,
                ...
                for temp in ANALYZED_TEMPERATURES
            }
            conditional_curves = fill_missing_temperature_curves(raw_curves)
...
def main() -> None:
    ...
    figure_path = RESULT_DIR / "Regional_fragility_curves_PFSDF_Yushu_Turpan_Wenchang.png"
