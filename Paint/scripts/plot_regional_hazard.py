...
用途：计算并对比多个城市的区域危险性曲线，输出总览图、局部放大图及计算表。
做法：读取各温度危险性曲线和城市温度权重，按权重合成区域曲线并进行特征点插值。
使用：修改“用户编辑区”中的 EDP、城市、温度、坐标范围和输出选项后运行本文件。
"""
...
# -----------------------------
# =============================================================================
# 用户编辑区：项目路径、模型、EDP、城市、温度、坐标轴和输出选项
# =============================================================================
...
CHINESE_FONT_FAMILY = ["SimSun", "Microsoft YaHei", "STSong", "DejaVu Serif"]
...
CITY_CONFIGS = [
    ...
    {"city": "20C", "label": f"20 {CELSIUS}", "color": "black", "linestyle": (0, (5, 3))},
]

CITY_LABELS = {str(config["city"]): str(config["label"]) for config in CITY_CONFIGS}
...
if EDP not in EDP_CONFIGS:
    raise ValueError(f"???? EDP ??: {EDP}????? {list(EDP_CONFIGS)}")
...
def configure_matplotlib() -> None:
    ...
    plt.rcParams["font.serif"] = ["Times New Roman", *CHINESE_FONT_FAMILY]
...
def load_temperature_weights(path: Path) -> pd.DataFrame:
    ...
    for city in df["city"].unique():
        ...
        if not np.isclose(city_prob_sum, 1.0, atol=1e-8):
            raise ValueError(f"{city} ?????????? 1???? {city_prob_sum:.10f}")
...
def save_figure(fig: plt.Figure, stem: str) -> None:
    ...
    fig.savefig(png_path, dpi=FIGURE_DPI, bbox_inches="tight", pil_kwargs={"compress_level": 1})
...
def plot_overview(curves: dict[str, pd.DataFrame], ds_configs: list[dict[str, float | str]]) -> None:
    ...
    ax.legend(
        loc="upper right",
        frameon=False,
        prop=FontProperties(family=CHINESE_FONT_FAMILY, size=18),
        handlelength=2.2,
    )
...
def plot_ds_zoom(curves: dict[str, pd.DataFrame], ds_name: str, threshold: float, zoom_half_width: float) -> None:
    ...
    ax.legend(
        loc="lower left",
        frameon=False,
        prop=FontProperties(family=CHINESE_FONT_FAMILY, size=20),
        handlelength=2.0,
    )
...
def export_summary_table(summary_df: pd.DataFrame) -> Path:
    ...
    header_font = Font(name="SimSun", size=14, bold=False)
    body_font = Font(name="SimSun", size=13, bold=False)
    border_top = Border(top=Side(style="medium", color="000000"))
    border_bottom = Border(bottom=Side(style="medium", color="000000"))

    cities = [str(config["city"]) for config in CITY_CONFIGS]
    ...
    for idx, (_, row) in enumerate(summary_df.iterrows(), start=data_start_row):
        ...
        for city in cities:
            ...
            lambda_value = row[f"{city_label}_lambda_EDP"]
            p50_value = row[f"{city_label}_P50 (%)"]
