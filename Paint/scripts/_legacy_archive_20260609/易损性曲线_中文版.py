...
def frag_curve_temp():
    ...
    for model in models:
        ...
        plt.savefig(root / 'Paint'  / f'Fragility_curves_{model}.png', dpi=600, bbox_inches='tight')
        # plt.show()
...
def frag_curve_3d():
    ...
    for dm_idx, DM in enumerate(DMs):
        for ds_idx in range(3):
            ...
            ax.set_xlabel(r'$Sa(T_1)$', fontsize=12, labelpad=5)
            ax.set_ylabel('Temperature (C)', fontsize=12, labelpad=5)
            ax.set_zlabel('Exceedance probability', fontsize=12, labelpad=5)
            ax.set_zlim(0, 1)
            ax.set_xlim(0, 1.5)
