...
# 手动配置区：只需要改这里
# =========================
PLOT_TYPE = "curves"      # "curves" | "surface" | "temp_slices"
MODEL = "SMABF"
EDP = "IDR"
DS = 3
...
# 配色
CMAP_NAME = "coolwarm"      # e.g., "plasma", "cividis", "coolwarm", "Spectral", "viridis" "RdYlBu"
SAVE_FIGURE = True
SHOW_FIGURE = True
SAVE_DPI = 300
# =========================
...
def _read_fragility_df(root: Path, model: str, temp: int, edp: str) -> pd.DataFrame | None:
    path = root / 'Output_data' / f'MC8_{model}_{temp}' / 'MC8_IDA_data_frag' / f'鏄撴崯鎬ф洸绾縚{edp}.xlsx'
    if not path.exists():
        return None
    # 浣犱箣鍓嶆槸 skiprows=1锛涙寜浣犵殑鏂囦欢鏍煎紡淇濈暀
...
def _extract_xy_for_temp(root: Path, model: str, temp: int, edp: str, ds: int, normalize_by_mce: bool = True):
    ...
    # 澶勭悊閲嶅 x锛氬彇 y 鏈€澶у€硷紙鏇寸ǔ锛屼究浜庢彃鍊硷級
...
def plot_fragility_curves():
    ...
    for temp in TEMPERATURES:
        ...
        if x is None:
            continue
        ax.plot(x, y * 100.0, lw=2.2, color=cmap(norm(temp)), label=f'{temp}掳C')

    # 鍧愭爣杞翠笌鍒诲害锛堟寜浣犵粰鐨勬牸寮忥級
...
def plot_fragility_surface():
    ...
    # 鏀堕泦鍚勬俯搴︿笅鐨勬洸绾匡紝骞舵彃鍊煎埌 x_common
    temps_used = []
    Z_list = []
    for temp in TEMPERATURES:
        x, y = _extract_xy_for_temp(root, MODEL, temp, EDP, DS, NORMALIZE_BY_MCE)
    ...
    # Interpolate along the temperature axis for a smoother surface.
    ...
    ax.set_zlabel('Probability of exceedance', fontsize=25, labelpad=12)
...
def plot_temp_slices():
    ...
    ax.tick_params(axis='both', direction='in', which='both', labelsize=18)
    ax.set_xlabel(r'Temperature ($^\circ$C)', fontsize=25, labelpad=12)
    ax.set_ylabel('Probability of exceedance (%)', fontsize=25, labelpad=12)
