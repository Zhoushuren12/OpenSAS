...
"""概率地震需求模型（PSDM）拟合图绘制工具。

用途：绘制 ln(DM)—ln(Sa) 散点、拟合直线，并标注拟合方程和统计参数。
做法：读取 ``概率需求模型_*.xlsx`` 与 ``概率特征_*.out``，批量生成不同温度和 EDP 的图件。
使用：修改“用户编辑区”中的模型、温度、EDP 和输出目录后运行本文件。
"""
...
BATCH_MODEL = "SMABF"
BATCH_TEMPERATURES = [-20, -10, 0, 10, 20, 30, 40]
BATCH_EDPS = ["IDR", "RIDR", "PFA"]
BATCH_OUTPUT_DIR = Path("Paint") / "PSDM输出"
...
if __name__ == "__main__":
    ...
    for temperature in BATCH_TEMPERATURES:
        for edp in BATCH_EDPS:
            model = f"{BATCH_MODEL}_{temperature}"
            save_path = BATCH_OUTPUT_DIR / f"PSDM_{model}_{edp}.png"
            main(
                model=model,
                DS=edp,
                save_path=save_path,
                color=BATCH_LINE_COLOR,
                show_plot=False,
            )
